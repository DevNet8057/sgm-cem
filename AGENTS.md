...
2. `.Codex/instructions/` — conventions par domaine (à lire selon la tâche) :
...
| `reviewer` | Relit le code selon les conventions SGM-CEM (`.Codex/instructions/`) |
| `git` | Branches, commits conventionnels français, propreté de l'historique |

Commandes : `/add-feature`, `/add-page`, `/add-api`, `/fix-bug` (voir `.Codex/commands/`).
Des hooks (`.Codex/hooks/`) lancent automatiquement type-check et ESLint après chaque écriture.
