...
const sessionQuery = pagination.extend({
  userId: z.string().uuid().optional(),
  state: z.enum(['ONLINE', 'INACTIVE', 'EXPIRED', 'LOGGED_OUT']).optional(),
})
...
router.get('/overview', authenticate, async (req, res) => {
  ...
  const now = Date.now(); const online = new Set(sessions.filter(s => !s.endedAt && s.authExpiresAt.getTime() >= now && now - s.lastSeenAt.getTime() < 90_000).map(s => s.userId))
...
router.get('/users', authenticate, async (req, res) => {
  ...
  const [users, total, sessions, actions] = await Promise.all([prisma.user.findMany({ skip, take: pageSize, select: { id: true, fullName: true, role: true, isActive: true, lastLoginAt: true }, orderBy: { fullName: 'asc' } }), prisma.user.count(), prisma.activitySession.findMany({ select: { userId: true, activeSeconds: true, endedAt: true, authExpiresAt: true, lastSeenAt: true } }), prisma.auditLog.findMany({ select: { userId: true } })])
  const data = users.map(u => { const ss = sessions.filter(s => s.userId === u.id); const now = Date.now(); return { userId: u.id, fullName: u.fullName, role: u.role, isActive: u.isActive, presence: ss.some(s => !s.endedAt && s.authExpiresAt.getTime() >= now && now - s.lastSeenAt.getTime() < 90000) ? 'ONLINE' : 'OFFLINE', lastSeenAt: ss.sort((a,b) => b.lastSeenAt.getTime()-a.lastSeenAt.getTime())[0]?.lastSeenAt.toISOString() ?? u.lastLoginAt?.toISOString() ?? null, openSessions: ss.filter(s => !s.endedAt).length, sessions: ss.length, activeSeconds: ss.reduce((n,s) => n+s.activeSeconds,0), actions: actions.filter(a => a.userId === u.id).length, failedLogins: 0 } })
  res.json({ success: true, data: { items: data, total, page, pageSize, totalPages: Math.ceil(total / pageSize) } })
})

router.get('/sessions', authenticate, async (req, res) => {
  await admin(req)
  const { page, pageSize, userId, state } = sessionQuery.parse(req.query)
  const now = new Date()
  const onlineSince = new Date(now.getTime() - 90_000)
  const stateWhere = state === 'LOGGED_OUT'
    ? { endedAt: { not: null } }
    : state === 'EXPIRED'
      ? { endedAt: null, authExpiresAt: { lt: now } }
      : state === 'ONLINE'
        ? { endedAt: null, authExpiresAt: { gte: now }, lastSeenAt: { gte: onlineSince } }
        : state === 'INACTIVE'
          ? { endedAt: null, authExpiresAt: { gte: now }, lastSeenAt: { lt: onlineSince } }
          : {}
  const where = { ...(userId ? { userId } : {}), ...stateWhere }
  const [sessions, total] = await Promise.all([
    prisma.activitySession.findMany({ where, orderBy: { lastSeenAt: 'desc' }, skip: (page - 1) * pageSize, take: pageSize, include: { tabs: true, pageAggregates: true } }),
  ...
  const items = sessions.map(session => {
    ...
    const sessionState = session.endedAt ? 'LOGGED_OUT' : session.authExpiresAt.getTime() < nowMs ? 'EXPIRED' : nowMs - session.lastSeenAt.getTime() < 90_000 ? 'ONLINE' : 'INACTIVE'
    return {
      id: session.id, userId: session.userId, userName: user?.fullName ?? 'Utilisateur supprimé', role: user?.role ?? 'INCONNU',
      startedAt: session.startedAt.toISOString(), lastSeenAt: session.lastSeenAt.toISOString(), lastActiveAt: session.lastActiveAt?.toISOString() ?? null, endedAt: session.endedAt?.toISOString() ?? null, authExpiresAt: session.authExpiresAt.toISOString(), state: sessionState, activeSeconds: session.activeSeconds,
